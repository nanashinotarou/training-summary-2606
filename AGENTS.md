...
**Cursor 設定**: `.cursor/rules/obsidian-hub-workflow.mdc`（alwaysApply）
...
**画像リネーム（プラン作成前に実行）:**
`assets/` に単純な連番ファイル（`1.png`, `2.png`, ...）があれば、Today_Research.md のDay番号を読み取って一括リネームする。

```
1.png → day01_slide1.png
2.png → day01_slide2.png
3.png → day01_slide3.png
```

リネーム後、Today_Plan.md の `[実装メタ情報]` に使用するファイル名を明記する。`assets/` に番号ファイルがなければこのステップはスキップ。

### Step 2-2：Cursorへの引き渡し
`Today_Plan.md` の更新完了後、Hiroyaに「プラン更新完了、Cursorに渡してください」と報告する。
...
- `assets/dayXX_*.png` が存在する場合、各セクションの冒頭または適切な位置に `<img>` タグで埋め込むこと
- 画像がない場合はテキストのみで実装する（画像待ちで止まらない）
- Today_Plan.md の `[実装メタ情報]` に使用する画像ファイル名を明記する
...
- **出力ファイル:** volXX-1.html
- **cache-bust:** YYYY-MM-DDTHH:MM:SS（Codexが当日日時で決定・明記する）
...
**運用上のポイント:**
- `cache-bust` の値はCodexが毎回明示する（Cursorが推測しない）
