...
## [ClaudeCodeレビューフィードバック ─ 2026-06-03]

ClaudeCodeによるvol02-1.htmlレビュー完了。全体の品質は高く、ほぼ合格。以下2点を次回以降の実装で改善すること。

### 🔴 指摘①：cache-bust の値を自己判断で変更しない（ルール違反）

**何が起きたか:** Today_Plan.md の `[実装メタ情報]` に `cache-bust: 2026-06-03T20:00:00` と明記していたにもかかわらず、実装では `2026-06-03T21:00:00` に変更されていた。

**ルール（CLAUDE.md §4 Cursor向け実装禁則）の再確認:**
> `cache-bust` の値は Today_Plan.md の `[実装メタ情報]` に記載された値をそのまま使うこと。自分で推測・変更しない。

**今後の対応:** cache-bust の値は ClaudeCode が決定・明記した値を**一字一句そのまま**使うこと。実装時刻との差異は気にしない。

---

### 🟡 指摘②：未使用スライド（slide13）の扱い ─ 確認・報告してから判断すること

**何が起きたか:** Today_Plan.md に記載のあった `assets/day02_slide13.png`（前半Summaryスライド「あなたのクリエイティブの武器が揃いました」）が、実装されたHTMLに含まれていなかった。

**なぜ問題か:** スライドの採否判断（「このスライドは使わない」）はCursorが単独で行うものではない。Today_Plan.md に記載されている素材を使わない場合は、**その理由をコメントに明記するか、ClaudeCodeに確認を取ること。**

今回の場合、まとめタブの役割がslide29に変わったためslide13が孤立したという状況は合理的だが、その判断を無言で行ったことが問題。

**今後の対応:**
- 計画に記載されたスライドを使わない場合は、コミットメッセージまたはToday_Plan.md末尾に `【未使用素材】day02_slide13.png ─ まとめタブをslide29に変更したため不使用` のように一行記録すること。
- 実装の迷い・判断をした場合は、**黙って判断せず記録に残す。**
